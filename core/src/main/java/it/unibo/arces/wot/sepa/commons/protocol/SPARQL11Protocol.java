...
- via POST directly
	...
	protected Response parseEndpointResponse(int token,String jsonResponse,SPARQLPrimitive op,QueryResultsFormat format) {
		...
		if (json.get("code") != null && json.get("body")!=null) {
			if (json.get("code").getAsInt() >= 400)
				return new ErrorResponse(token,json.get("code").getAsInt(),json.get("body").toString());
		}
		
		if (op.equals(SPARQLPrimitive.UPDATE)) return new UpdateResponse(token,json.get("body").toString());
